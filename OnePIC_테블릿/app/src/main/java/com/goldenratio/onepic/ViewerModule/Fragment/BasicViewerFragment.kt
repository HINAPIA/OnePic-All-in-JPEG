...
//            }, 200) // 포커스 설정을 지연시키기 위해 postDelayed 사용 (필요에 따라 딜레이 시간 조정)
    ...
    fun init() {
        ...
        val paddingStart = itemWidth-50//pxToDp(requireContext(),90f)//itemWidth - 90//90//pxToDp(requireContext(),((screenWidth - itemWidth) / 2 + 170).toFloat()).toInt()//tab 에서 되는 코드 -> dpToPx(170f,requireContext())
